...
class MainWindow(wx.Dialog):
    def __init__(self, parent, title, instanceData=None):
        ...
        if not self.instanceData:
            try:
                ...
                wx.MessageBox("Não foi possível mover seu histórico de pesquisa anterior para a nova pasta utilizada pelo programa. Caso ainda deseje utilizar este histórico, mova-o manualmente da pasta principal do programa para a subpasta data.", "Erro ao mover o histórico", wx.OK | wx.ICON_ERROR)
            self.notifList=[]
                    ...
                    def onSeeReplies(event):
                        ...
                        def onRepliesLoaded(event):
                            ...
                            def onReplyChange(event):
                                ...
                                formatedText=formatText(replyData["originalText"])
    ...
    def getFeedData(self, entry):
        ...
        publishedAt=datetime.strptime(originalPublishedAt, "%Y-%m-%dT%H:%M:%S+%f:00")
        publishedAt=convertTimezone(publishedAt)
        videoData={
    ...
    def initUi(self):
        ...
        def onSettings(event):
            ...
            languageName = list(languageDict.keys())[list(languageDict.values()).index(languageCode)]
            transcriptLanguageBox.SetStringSelection(languageName)
            autoTranslateTranscriptsLabel = wx.StaticText(settingsDial, label="Tradu&zir as transcrições automaticamente caso necessário")
            autoTranslateTranscripts = wx.CheckBox(settingsDial)
            autoTranslateTranscripts.SetValue(self.getSetting("auto_translate_transcripts"))
            ...
            def onOk(event):
                ...
                self.setSetting("auto_translate_transcripts", autoTranslateTranscripts.GetValue())
